...
class CometDeployer {
    ...
    injectServers(shipit, config) {
        return this.cometAws.describeInstancesWithFilters(this.tagFilters).then( instances => {
            return instances.map( instance => {
                if (this.usePrivateDns) {
                    return `${this.sshUsername}@${CometAws.getPrivateDnsNameForInstance(instance)}`;
                } else {
                    return `${this.sshUsername}@${CometAws.getPublicDnsNameForInstance(instance)}`;
                }
    ...
    configureShipit(shipit, config) {
        ...
        this.injectErrorHandler(shipit);
        return this.injectServers(shipit, config);
    }
}
